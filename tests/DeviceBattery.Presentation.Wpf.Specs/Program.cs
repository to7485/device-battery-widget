...
var specs = new (string Name, Action Run)[]
{
    ...
    ("BLE device is projected beside preferred DualSense", () => { var vm = new WidgetViewModel(); BatteryState dualSenseBattery = BatteryState.Available(15, ChargingState.NotCharging, BatteryPrecision.TenPercentBucket, now, key.ProviderId); DeviceKey ble = new("BleGattBattery", "ble-one"); BatteryState bleBattery = BatteryState.Available(73, ChargingState.Unknown, BatteryPrecision.ExactPercent, now, ble.ProviderId); vm.Apply(Applied(new(key, "DualSense Wireless Controller", dualSenseBattery, true, 1))); vm.Apply(Applied(new(ble, "AULA-F87Pro 5.0", bleBattery, true, 1))); Equal(2, vm.Devices.Count); Equal(ble, vm.Devices[1].Key); Equal("73%", vm.Devices[1].BatteryText); }),
    ("Xbox granular battery is marked estimated", () => { DeviceKey xbox = new("WindowsGamingInputBattery", "xbox-one"); BatteryState battery = BatteryState.Available(10, ChargingState.NotCharging, BatteryPrecision.GranularLevel, now, xbox.ProviderId); var card = new DeviceCardViewModel(new(xbox, "Xbox Wireless Controller", battery, true, 1)); Equal("약 10%", card.BatteryText); }),
    ("BLE Xbox battery suppresses duplicate Gaming Input row", () => { var vm = new WidgetViewModel(); DeviceKey wgi = new("WindowsGamingInputBattery", "xbox-wgi"); DeviceKey ble = new("BleGattBattery", "xbox-ble"); vm.Apply(Applied(new(wgi, "Xbox Wireless Controller", BatteryState.Available(10, ChargingState.NotCharging, BatteryPrecision.GranularLevel, now, wgi.ProviderId), true, 1))); vm.Apply(Applied(new(ble, "Xbox Wireless Controller", BatteryState.Available(70, ChargingState.Unknown, BatteryPrecision.ExactPercent, now, ble.ProviderId), true, 1))); Equal(1, vm.Devices.Count); Equal(ble, vm.Devices[0].Key); Equal("70%", vm.Devices[0].BatteryText); }),
    ("Saved position is clamped to working area", () => { Point restored = WindowPositionPolicy.Restore(1900, 1000, new Size(360, 120), [new Rect(0, 0, 1920, 1040)]); Equal(new Point(1560, 920), restored); }),
    ("Off-screen position falls back to primary working area", () => { Point restored = WindowPositionPolicy.Restore(5000, 5000, new Size(360, 120), [new Rect(10, 20, 1000, 700)]); Equal(new Point(650, 600), restored); })
};
